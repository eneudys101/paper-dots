...
def encode_string(s):
    try:
        ...
        while i < len(s):
            ch = s[i]
            if 0xD800 <= ord(ch) <= 0xDBFF and i + 1 < len(s):  # Check if it's a high surrogate
                # Encode the complete character (both halves of the surrogate pair)
                parts.append(s[i:i+2].encode('utf-16-le'))
                i += 2
            else:
                parts.append(ch.encode('utf-8'))
                i += 1
        return b''.join(parts)
