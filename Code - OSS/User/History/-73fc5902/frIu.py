...
def clean_string(s):
    return ''.join(ch for ch in s if unicodedata.category(ch) != 'Cs' and ord(ch) < 0x10000)
''.join(ch for ch in s if ord(ch) < 0x10000)
...
def on_window_focus(i3, e):
    ...
    if prefix:
        ...
        print(clean_string(f'rename workspace to "{prefix}:{new_name}"'))
