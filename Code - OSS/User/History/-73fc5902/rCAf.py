...
def encode_string(s):
    try:
        ...
        # Ensure that surrogates are paired
        encoded_parts = []
        i = 0
        while i < len(s):
            if 0xD800 <= ord(s[i]) <= 0xDBFF and i + 1 < len(s) and 0xDC00 <= ord(s[i+1]) <= 0xDFFF:  # Check if it's a valid surrogate pair
                encoded_parts.append(s[i:i+2].encode('utf-16-le'))
                i += 2
            else:
                encoded_parts.append(s[i].encode('utf-8'))
                i += 1
...
def shift_workspaces_up(i3):
    ...
    for ws in workspaces:
        ...
        if prefix:
            ...
            print(clean_string(f'rename workspace "{ws.name}" to "{new_prefix}:{name}"'))

def on_window_focus(i3, e):
    prefix, _ = extract_prefix_and_name(i3.get_tree().find_focused().workspace().name)
    if prefix:
        new_name = get_app_name(i3)
        print(encode_string(f'rename workspace to "{prefix}:{new_name}"'))

def on_new_workspace_init(i3, e):
    shift_workspaces_up(i3)
    print(clean_string(f'rename workspace "{e.current.name}" to "1: Empty"'))
...
def initialize_workspaces(i3):
    ...
    for index, ws in enumerate(workspaces, start=1):
        ...
        print(encode_string(f'rename workspace "{ws.name}" to "{index}:{app_name}"'))
