...
def get_app_name(i3):
    ...
    return APP_NAME_MAPPING.get(app_id, "\uf128")
...
def on_window_focus(i3, e):
    ...
    if not focused or not focused.workspace():
        return
    prefix, _ = extract_prefix_and_name(focused.workspace().name)
    if prefix:
...
def on_new_workspace_init(i3, e):
    ...
    i3.command(f'rename workspace "{e.current.name}" to "1: \uf128"')
...
def initialize_workspaces(i3):
    ...
    for index, ws in enumerate(workspaces, start=1):
        ...
        app_name = APP_NAME_MAPPING.get(app_node.app_id if app_node else None, "\uf128")
